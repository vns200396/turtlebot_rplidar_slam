...
int16_t Controller::get_acceleration_rawX(void){
	...
	rawX = imu_driver.Acceleration_RawX;
	return (rawX);
}
...
int16_t Controller::get_acceleration_rawY(void){
	...
	rawY = imu_driver.Acceleration_RawY;
	return (rawY);
}
...
int16_t Controller::get_acceleration_rawZ(void){
	...
	rawZ = imu_driver.Acceleration_RawZ;
	return (rawZ);
}

double Controller::get_acceleration_X(void)
{
	signed int short raw;
	signed int acceleration;
	raw = (signed int short) get_acceleration_rawX();
	acceleration = (signed int)((signed int)raw);
	return (1.0*acceleration)/256;
}

double Controller::get_acceleration_Y(void)
{
	signed int short raw;
...
double Controller::get_acceleration_Z(void)
{
	signed int short raw;
	signed int acceleration; // = (signed int)(((signed int)raw) * 3.9);
...
double Controller::getPitch(void)
{				
	double result;
...
double Controller::getRoll(void)
{
	double result;
	result =atan(get_acceleration_Y() / sqrt(pow(get_acceleration_X(), 2) + pow(get_acceleration_Z(), 2)));
	result = (result * 180) / PI;
...
double Controller::getYaw(void)
{
	double result;
	result = atan((sqrt(pow(get_acceleration_X(), 2) + pow(get_acceleration_Y(), 2)))/get_acceleration_Z());
	result = (result * 180) / PI;
	return result;
}
...
void Controller::requestController(ros::Duration tRequset)
{
	...
	if(time_now >= nextTrq)
	{
		...
		case CMD_FAULT_CLEAR:
			cmdSetStatusDriver2(cmd,STOP_MOTOR, STOP_MOTOR,_wheelLeft->getDesiredDir(), _wheelRight->getDesiredDir(), 0, 0);
			break;
		default:
			break;
		}
		nextTrq = time_now + tRequset;
	}
}

uint8_t Controller::controllerRespond()
{
	...
	if(result == COMM_OK)
	{
		...
		case CMD_SET_SST_DRIVER2:
			...
			break;
		case CMD_REQUEST_ACC_IMU:
			get_ACC_IMU(&imu_driver, driver_packet.data, LengPayLoad);
			break;
		case CMD_REQUEST_SONAR_ENC:
...
unsigned int Controller::setRadiusMM(int radiusMM) {
	_radiusMM=radiusMM;
	return getRadiusMM();
}

unsigned int Controller::wheelRightSetSpeedMMPS(int speedMMPS,bool dir) {
...
unsigned int Controller::wheelLeftSetSpeedMMPS(int speedMMPS,bool dir) {
...
unsigned int Controller::setMotorAll(int speedMMPS,bool dir) {
...
unsigned int Controller::setMotorAllAdvance(int speedMMPS) {
	return setMotorAll(speedMMPS,DIR_ADVANCE);
}

unsigned int Controller::setMotorAllBackoff(int speedMMPS) {
...
unsigned int Controller::setCarRotateRight(int speedMMPS) {
...
unsigned int Controller::setCarArcBase(int speedMMPS,int radiusMM) {
	int delta=	radiusMM;//(int)((float)getWheelspanMM()/(radiusMM<<1)*speedMMPS);
	int V1= abs(speedMMPS-delta);
	int V2= abs(speedMMPS+delta);
...
unsigned int Controller::setCarUpperLeft(int speedMMPS,int radiusMM) {
	setCarStat(STAT_UPPERLEFT);
	return setCarArcBase(speedMMPS,radiusMM);
}

unsigned int Controller::setCarLowerLeft(int speedMMPS,int radiusMM) {
	setCarStat(STAT_LOWERLEFT);
	return setCarArcBase(speedMMPS,radiusMM);
}

unsigned int Controller::setCarLowerRight(int speedMMPS,int radiusMM) {
	setCarStat(STAT_LOWERRIGHT);
	return setCarArcBase(speedMMPS,radiusMM);
}

unsigned int Controller::setCarUpperRight(int speedMMPS,int radiusMM) {
	setCarStat(STAT_UPPERRIGHT);
	return setCarArcBase(speedMMPS,radiusMM);
}

unsigned int Controller::setCarSpeedMMPS(int speedMMPS,unsigned int ms) {
	unsigned int carStat=getCarStat();
	unsigned int currSpeed=getCarSpeedMMPS();

	unsigned int (Controller::*carAction)(int speedMMPS);
	switch(carStat) {
		case STAT_ADVANCE:
...
unsigned int Controller::setCarSpeedMMPSArc(int speedMMPS,int radiusMM,unsigned int ms) {
	unsigned int carStat=getCarStat();
	unsigned int currSpeed=getCarSpeedMMPS();

	unsigned int (Controller::*carAction)(int speedMMPS,int radiusMM);
...
unsigned int Controller::setCarArcAngle(int speedMMPS,int radiusMM,float radian,unsigned int uptime) {
	if(radian<0) return 0;
	radiusMM= (unsigned int)(radiusMM*radian);
	setCarSpeedMMPSArc(speedMMPS,radiusMM,uptime);
	//delayMS(timeMS-uptime);
	//setCarSlow2Stop(uptime);
	return radiusMM;
}
unsigned int Controller::setCarUpperLeftAngle(int speedMMPS,int radiusMM,float radian,unsigned int uptime) {
	setCarUpperLeft(0,radiusMM);
	return setCarArcAngle(speedMMPS,getRadiusMM(),radian,uptime);
}
unsigned int Controller::setCarLowerLeftAngle(int speedMMPS,int radiusMM,float radian,unsigned int uptime) {
	setCarLowerLeft(0,radiusMM);
	return setCarArcAngle(speedMMPS,getRadiusMM(),radian,uptime);
}
unsigned int Controller::setCarUpperRightAngle(int speedMMPS,int radiusMM,float radian,unsigned int uptime) {
	setCarUpperRight(0,radiusMM);
	return setCarArcAngle(speedMMPS,getRadiusMM(),radian);
}
unsigned int Controller::setCarLowerRightAngle(int speedMMPS,int radiusMM,float radian,unsigned int uptime) {
...
void Controller::reset_encoders()
{
	requestCommand(CMD_RESET_ENCODER);
}
...
void Controller::controller_debugger(void)
{
	...
	// 	time_begin = ros::Time::now().toSec();
		...
		// ROS_INFO("Kp= %f Ki= %f Kd= %f", _wheelLeft->GetKp(), _wheelLeft->GetKi(), _wheelLeft->GetKd());
